...
export default function QuizSetup({hasDocuments, setSourceType, category, setCategory, onStart, loading, resumeSession, onResume}: QuizSetupProps) {
  const [mode, setMode] = useState<'mixed' | 'topic' | 'documents'>('mixed')
  ...
  return <div className="mx-auto w-full max-w-3xl px-5 py-8 sm:py-12 text-[#0B2545]">
    ...
    <h1 className="mt-3 text-3xl font-bold sm:text-4xl">Make time for five questions.</h1>
    ...
    <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 sm:p-7">
      ...
      <p className="mt-6 text-sm text-slate-600">5 questions · Go at your own pace{mode==='documents' ? ' · From your notes' : ''}</p>
      ...
      <button disabled={loading || (mode==='documents' && !hasDocuments)} onClick={()=>onStart({quizMode:'standard',totalQuestions:5})} className="mt-4 min-h-12 w-full rounded-xl bg-[#0D8F9C] px-5 py-3 font-bold text-white disabled:opacity-50">{loading ? 'Starting…' : 'Start 5 questions →'}</button>
    </section>
    <Link href="/entry" className="mt-6 inline-block text-sm font-semibold text-[#087986] underline">See my suggested focus on Home</Link>
