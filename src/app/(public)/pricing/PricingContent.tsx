...
export default function PricingContent({ isBeta, isSubscribed }: PricingContentProps) {
  ...
  // Already subscribed — no pricing to show
  if (isSubscribed) {
    return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold bg-gradient-to-r from-slate-900 via-indigo-900 to-slate-900 bg-clip-text text-transparent mb-4">
            {isBeta ? 'Your Founder Pricing' : 'Choose Your Plan'}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto mb-8">
          ...
          >
            ...
            <div className="mb-4">
              {isBeta ? (
                <>
                  <span className="text-4xl font-bold text-teal-700">$14.99</span>
                  <span className="text-lg text-slate-500"> / month</span>
                  <span className="ml-2 text-sm text-slate-400 line-through">$24.99</span>
                </>
              ) : (
                <>
            ...
            <ul className="space-y-2.5 mb-6 text-sm text-slate-700">
              <li className="flex items-start gap-2"><span className="text-teal-600 mt-0.5">•</span><span>Unlimited access to ForgeNursing</span></li>
              <li className="flex items-start gap-2"><span className="text-teal-600 mt-0.5">•</span><span>Billed monthly</span></li>
          ...
          >
            ...
            {isBeta ? (
              ...
              <p className="text-sm text-indigo-700 mb-4 font-semibold">Save $11 vs monthly</p>
            )}
            ...
            <ul className="space-y-2.5 mb-6 text-sm text-slate-700">
              ...
              <li className="flex items-start gap-2"><span className="text-teal-600 mt-0.5">•</span><span>No contracts</span></li>
              <li className="flex items-start gap-2"><span className="text-teal-600 mt-0.5">•</span><span>No surprise renewals</span></li>
              {isBeta && <li className="flex items-start gap-2"><span className="text-teal-600 mt-0.5">•</span><span className="font-medium text-teal-700">Founder rate — locked forever</span></li>}
              {!isBeta && <li className="flex items-start gap-2"><span className="text-teal-600 mt-0.5">•</span><span>7-day free trial included</span></li>}
            </ul>
            {selectedPlan === 'semester' && (
          ...
          >
            ...
            <div className="mb-1">
              {isBeta ? (
                <>
                  <span className="text-4xl font-bold text-teal-700">$129</span>
                  <span className="text-lg text-slate-500"> / year</span>
                  <span className="ml-2 text-sm text-slate-400 line-through">$199</span>
                </>
              ) : (
                <>
            ...
            {isBeta ? (
              <p className="text-sm text-teal-700 mb-4 font-semibold">Founder rate — save $70 vs standard</p>
            ) : (
              <p className="text-sm text-indigo-700 mb-4 font-semibold">Save 34% vs monthly</p>
            )}
            <p className="text-sm text-slate-700 mb-4 font-medium">For students committed to mastery and peace of mind.</p>
            <ul className="space-y-2.5 mb-6 text-sm text-slate-700">
        ...
        <div className="text-center max-w-md mx-auto">
          ...
          >
            {loading ? (
              ...
              <>
                {isBeta ? 'Lock In My Founder Rate' : 'Continue to Checkout'}
