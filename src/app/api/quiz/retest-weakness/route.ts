...
function normalizeQuestion(raw: any, category: string, mistakeType: MistakeType, retestFocus: string) {
  ...
  if (new Set(parsed.options.map(option => option.label)).size !== 4 || parsed.options.some(option => !labels.includes(option.label) || !option.text.trim())) throw new Error('Invalid option labels');
  if (labels.some(label => label !== parsed.correct_answer && !parsed.rationale_incorrect?.[label as 'A' | 'B' | 'C' | 'D']?.trim())) throw new Error('Missing option rationale');
  return {
    ...parsed,
  ...
  try {
    ...
    const { data: retestSession, error: sessionError } = await supabase
      ...
      .insert({
        ...
        source_type: 'generic',
        quiz_mode: 'retest',
        target_mistake_type: mistakeType,
        target_focus: retestFocus,
        nclex_category: category,
        status: 'in_progress',
        score: 0,
        total_questions: 1,
