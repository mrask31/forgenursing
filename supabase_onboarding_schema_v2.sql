-- Migration V2: Add onboarding tracking fields to profiles table
-- Purpose: Track user progress through 3-step onboarding flow
-- Date: 2026-02-01
-- IMPROVED: Only marks users who signed up BEFORE today as completed
...
-- IMPROVED: Only mark users who signed up BEFORE today as completed
-- This prevents new signups from being marked as completed
UPDATE profiles 
...
WHERE id IN (
  ...
  WHERE created_at < CURRENT_DATE  -- Only users who signed up before today
)
